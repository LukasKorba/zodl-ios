...
extension TransactionGuardClient {
    ...
    func withSubmission<T>(_ body: @Sendable () async throws -> T) async rethrows -> T {
        await acquire()
        do {
    ...
    func switchIfIdle(_ body: @Sendable () async throws -> Void) async rethrows -> Bool {
    ...
    func switchWaiting(_ body: @Sendable () async throws -> Void) async rethrows {
        await acquire()
        do {
            try await body()
            await release()
